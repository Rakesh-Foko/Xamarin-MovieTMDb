...
namespace TheMovie.ViewModels
{
    ...
    {        
        ...
        public DelegateCommand LoadDiscoverMoviesCommand { get; }
        ...
        public DelegateCommand FavoriteButtonClickedCommand { get; }
        public DelegateCommand ReviewButtonClickedCommand { get; }

        private readonly INavigationService navigationService;
        public MainPageViewModel(INavigationService navigationService)
        {
            Title = "IEG TMDb - Discover Movies List";
            this.navigationService = navigationService;

            LoadDiscoverMoviesCommand = new DelegateCommand(async () => await ExecuteLoadDiscoverMoviesCommand().ConfigureAwait(false));
            ...
            FavoriteButtonClickedCommand = new DelegateCommand(async () => await ExecuteFavoriteMoviesCommand().ConfigureAwait(false));
            ReviewButtonClickedCommand = new DelegateCommand(async () => await ExecuteReviewButtonClickedCommand().ConfigureAwait(false));

            Movies = new ObservableRangeCollection<Movie>();

             LoadDiscoverMoviesCommand.Execute();
        }

        private async Task ExecuteFavoriteMoviesCommand()
        {
            await navigationService.NavigateAsync("FavoritesPage").ConfigureAwait(false);
        }

        private async Task ExecuteReviewButtonClickedCommand()
        {
            await navigationService.NavigateAsync("ReviewPage").ConfigureAwait(false);
        }


        private async Task ExecuteLoadDiscoverMoviesCommand()
        {
            ...
            try
            {
                Movies.Clear();
                currentPage = 1;
                await LoadMoviesAsync(currentPage, Enums.MovieCategory.Discover).ConfigureAwait(false);
            }
            finally
            {
                IsBusy = false;
